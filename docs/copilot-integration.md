...
**Do not edit this file manually.** The SupremePower Active Engine (`sp-context-bridge`) manages it.
...
1.  **You Start a Skill:** You run `/supremepower:tdd` (or start via VS Code).
2.  **State Update:** The engine initializes the session and sets the step to "RED".
3.  **Bridge Execution:** The `sp-context-bridge` script reads the TDD skill, extracts the "RED" step instructions, and writes them to `.github/supremepower-active.md`.
